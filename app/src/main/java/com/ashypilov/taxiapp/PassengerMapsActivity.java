...
public class PassengerMapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    private DatabaseReference driversGeoFile;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        driversGeoFile = FirebaseDatabase.getInstance().getReference().child("driversGeoFile");
    ...
    private void gettingNearestTaxi() {
        bookTaxiButton.setText("Getting your taxi..." + searchRadius);
        GeoFire geoFire = new GeoFire(driversGeoFile);
    ...
    private void getNearestDriverLocation() {
        ...
        nearestDriverLocation = FirebaseDatabase.getInstance().getReference().child("driversGeoFile").child(nearestDriverId).child("l");
        nearestDriverLocation.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    ...
                    driverMarket = mMap.addMarker(new MarkerOptions().position(driverLatLng).title("Your driver is here"));
                }
            }
    ...
    private void signOutPassenger() {
        ...
        DatabaseReference passenger = FirebaseDatabase.getInstance().getReference().child("passenger");
            ...
            case CHECK_SETTINGS_CODE:
                ...
                switch (resultCode) {
                    case Activity.RESULT_OK:
                        Log.d("MainActivity", "User change location");
                        startLocationUpdate();
                        break;

                    case Activity.RESULT_CANCELED:
                        Log.d("MainActivity", "User has not change location");
                        isLocationUpdate = false;
                        updateLocationUi();
                        break;
                }
    ...
    private void updateLocationUi() {
        ...
        if (currentLocation != null) {
            ...
            String passengerUserIdGeoFire = currentUser.getUid();
            DatabaseReference passengerGeoFire = FirebaseDatabase.getInstance().getReference().child("passengerGeoFire");
            DatabaseReference passenger = FirebaseDatabase.getInstance().getReference().child("passenger");
            passenger.setValue(true);
            GeoFire geoFire = new GeoFire(passengerGeoFire);
            geoFire.setLocation(passengerUserIdGeoFire, new GeoLocation(currentLocation.getLatitude(), currentLocation.getLongitude()));
        }
    }
    ...
    protected void onResume() {
        ...
        if (isLocationUpdate && checkLocationPermission()) {
            startLocationUpdate();
        }
...
//        super.onRequestPermissionsResult(requestCode, permissions, grantResults);
        if (requestCode == REQUEST_LOCATION_PERMISSION) {
            if (grantResults.length <= 0) {
                Log.d("MainActivity", "Request was cancelled");
            }
